...
import { build as viteBuild, createServer as createViteServer, mergeConfig as mergeViteConfig, } from 'vite';
...
export async function build(ctx) {
    ...
    const buildEndCallbacks = [];
    const previewStoryCallbacks = [];
...
function generateBaseHtml(head, body, ctx) {
    return `<!DOCTYPE html>
<html>
<head>
...
function generateEntryHtml(jsEntryFile, cssEntryFile, variables, ctx) {
    return generateBaseHtml(`<link rel="stylesheet" href="${ctx.resolvedViteConfig.base}${cssEntryFile}">
    ${ctx.config.theme?.favicon ? `<link rel="icon" type="${lookupMime(ctx.config.theme.favicon)}" href="${ctx.config.theme.favicon}"/>` : ''}
    ${variables.HEAD ?? ''}`, `<div id="app"></div>
    <script type="module" src="${ctx.resolvedViteConfig.base}${jsEntryFile}"></script>`, ctx);
}
async function writeFile(fileName, content, ctx) {
    await fs.writeFile(join(ctx.config.outDir, fileName), content, 'utf8');
}
function generateScriptLinks(prefetchScripts, rel, ctx) {
