...
import { mergeConfig as mergeViteConfig, loadConfigFromFile as loadViteConfigFromFile, } from 'vite';
...
export async function mergeHistoireViteConfig(viteConfig, ctx) {
    if (ctx.config.vite) {
        const command = ctx.mode === 'dev' ? 'serve' : 'build';
        const overrides = typeof ctx.config.vite === 'function'
            ? await ctx.config.vite(viteConfig, {
                mode: ctx.mode,
                command,
            })
            : ctx.config.vite;
...
export async function getViteConfigWithPlugins(isServer, ctx) {
