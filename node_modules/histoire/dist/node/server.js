...
export async function createServer(ctx, options = {}) {
    ...
    const [server, nodeServer, , { stop: stopMdFileWatcher },] = await Promise.all([
        getViteServer(false),
        getViteServer(true),
        watchStories(ctx),
    ...
    const { clearCache, executeStoryFile, destroy: destroyCollectStories, clearInvalidates, } = useCollectStories({
        server: nodeServer,
        mainServer: server,
    }, ctx);
